...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Sapphire is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Sapphire is downloading blocks...");
...
Value getworkaux(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Sapphire is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Sapphire is downloading blocks...");
